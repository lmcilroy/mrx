...
void
avx_hash4_start_seed(avx_hash4_state_t * const state,
...
void
avx_hash4_start(avx_hash4_state_t * const state)
{
	uint64_t seed[4] = { 0, 0, 0, 0 };

	avx_hash4_start_seed(state, seed);
}

void
avx_hash4_update(avx_hash4_state_t * const state, const void * const data,
    const size_t len)
{
	__m256i *v;
...
void
avx_hash4_end(avx_hash4_state_t * const state, avx_hash4_t * const hash)
{
	...
	avx_hash4_update(state, padding, len);
	avx_hash4_update(state, &total_len, 8);
	...
	h = _mm256_extractf128_si256(hx, 0);
	h = combine128(h, _mm256_extractf128_si256(hx, 1));
	h = fmix128(h);
...
void
avx_hash4_seed(const void * const data, const size_t len,
    const void * const seed, avx_hash4_t * const hash)
{
	avx_hash4_state_t avx_hash;

	avx_hash4_start_seed(&avx_hash, seed);
	avx_hash4_update(&avx_hash, data, len);
	avx_hash4_end(&avx_hash, hash);
}

void
avx_hash4(const void * const data, const size_t len,
    avx_hash4_t * const hash)
{
	uint64_t seed[4] = { 0, 0, 0, 0 };

	avx_hash4_seed(data, len, seed, hash);
}
